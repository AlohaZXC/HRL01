...
class SubgoalDiscovery():
	...
	def f_k_c1(self):
		...
		for k in range(2,8):
			kk = KMeans(n_clusters=k,init='random',max_iter=300)
			kk.fit(self.X)
			sil_score.append(silhouette_score(self.X,kk.labels_))
		print(sil_score)
		key = np.argmax(sil_score)
		key = key+2
		self.keynumer.append(key)
		print("..........", key)
	def f_k_c2(self):
		k = max(self.keynumer)
		print("final",k)
		#是否就这几个就够了？需要在之间加一些连接性的点作为额外辅助吗？
		self.kmeans = KMeans(n_clusters=k, init='random', max_iter=300)
		self.kmeans.fit(self.X)
		self.C = self.cluster_centroids()
		self.centroid_memory.append(self.C)
		self.centroid_subgoals = [ tuple(g) for g in list(self.C) ]
	...
	def f_k_c3(self):#是否就这几个就够了？需要在之间加一些连接性的点作为额外辅助吗？
		k = max(self.keynumer)
		print("final",k)
		...
		for i in range(k):
			...
			for j in range(i+1,k):
				x = (tempc[i][0]+tempc[j][0])/2
				y = (tempc[i][1]+tempc[j][1])/2
				temp.append(x,y)
			Temp_t.append(temp)
		flag = 1/16
		for i in range(tempc):
			for j in range(Temp_t):
				F = []
				d = np.sum(tempc[i][0]-Temp_t[j][0])+np.sum(tempc[i][1]-Temp_t[j][1])
				if d<=flag:
					F.append()
		# 1.让两个中心点扩大半径，求交点或者交点区域中出现次数最多的点
		# 2.两个中心点，以垂直于二者连线的直径做四边形，去其中出现次数最多的点
		# 3. 两个中心点，取中间附近区域某出现次数最多的点
	...
	def push_outlier(self, outlier,threshold=1):
		...
		if min(distance) >= threshold:
			...
			#print(outlier)
		 	...
		 	#print('discovered outlier already in the outliers list:',self.outliers)
			1
	def p_o(self):
		# 假设找到了钥匙，
		key = (10,13)
		b = (2,11)
		self.outliers.append(key)
		self.outliers.append(b)
		self.G = self.centroid_subgoals + self.outliers
